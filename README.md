...
| `execute:wave:post` | verifier | Flag drift from the chosen mechanism, dropped edge cases, unstable substitutions, unexplained constants, unsupported performance claims, an unreachable or unwritten branch, an argument whose accepted values are undocumented at its definition, agent-invoked code that is not quiet, prose that is not legible, and code that does not go with the grain of the project's conventions. |
...
A global install publishes those bytes to every project on the machine, so the hook installs only bytes it can show are already published. It refuses when Git is unusable, when the bundle holds uncommitted or gitignored files, when neither `origin/HEAD` nor `origin/main` exists to prove publication, and when the bundle's `HEAD` is not an ancestor of that ref. Running the test suite trips the second one: it leaves `__pycache__/` inside the bundle, which `git status` calls clean but a directory copy would still publish. A refusal names its reason on stderr, installs nothing, and leaves the recorded hash unwritten, so the next session retries once the bundle is published. The check covers only a bundle its enclosing repository tracks, and whether a marketplace install is one of those depends on the host. Claude Code has materialised plugin caches both as depth-1 git clones and as plain directories; both forms exist side by side in a single cache today. A plain directory is untracked and installs unchecked. A clone is tracked, so the check applies — and it refuses, because the plugin host writes its own `.in_use/` and `.orphaned_at` bookkeeping inside the cache directory and the uncommitted-or-ignored test counts those. Nothing is then installed at global scope, so a project with no `.gsd/capabilities/sota-numerics` copy of its own sees the gate report a missing script until you run `gsd-tools capability install <cache-path> --scope global --yes` by hand.
