...
| The bundle holds uncommitted or gitignored files. | `sota-numerics bundle has uncommitted or ignored files` | Commit them. Running the test suite trips this: it leaves `__pycache__/` inside the bundle, and the guard's `git status --ignored` reports that directory by name -- that is what makes the guard fire. Delete it. |
